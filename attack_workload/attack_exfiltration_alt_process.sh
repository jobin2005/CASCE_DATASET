...
psql -U postgres -d casce_tpcb -c "$(casce_sql_prelude)COPY (SELECT * FROM pgbench_accounts LIMIT 500) TO PROGRAM 'python3 -c \"import urllib.request, sys; req = urllib.request.Request(\\\"http://127.0.0.1:9090\\\", data=sys.stdin.read().encode(), method=\\\"POST\\\"); urllib.request.urlopen(req)\" > /dev/null 2>&1 || true';"
